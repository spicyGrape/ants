...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self,min_range = 0, max_range = float('inf')):
        ...
        def find_nearest_bees(place: Place, distance = 0):
            if place.is_hive == True:
                return [],distance

            # if no valid bees
            if place.bees == [] or distance < min_range:
                if place.entrance is not None:
                    distance += 1
                    return find_nearest_bees(place.entrance, distance)
                else:
                    return [], distance
            return place.bees, distance
        target_bees, target_distance = find_nearest_bees(self.place)
        target_bee = random_bee(target_bees)
        if min_range <= target_distance <= max_range:
            return target_bee
        else:
            return None
        # END Problem 3 and 4
    ...
    def action(self, gamestate):
        ...
        self.throw_at(self.nearest_bee(self.min_range, self.max_range))
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other:Ant):
        # BEGIN Problem 8
        if self.is_container and not(other.is_container) and self.ant_contained == None:
            return True
        else:
            return False
        # END Problem 8
...
class TankAnt(ContainerAnt):
    ...
    def __init__(self, health = initial_health):
        super().__init__(health)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.do_QueenAnt_exist == False:
            gamestate.do_QueenAnt_exist = True
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if self.scared_timer is 0 and self.slowed_timer is 0:
            destination = self.place.exit
        elif self.slowed_timer > 0 and self.scared_timer == 0 and int(gamestate.time % 2) == 0:
            destination = self.place.exit
        elif self.slowed_timer > 0 and int(gamestate.time % 2) == 1:
            destination = None
        elif (self.scared_timer) > 0 and (self.place.entrance is not None) and (self.place.entrance.is_hive == False):
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target: Bee):
        # BEGIN Problem EC
        if target:
            target.scare(self.scare_length)
        # END Problem EC
